...
# ---------------------------------------------------------------------------
# Allowed Applications Configuration
# ---------------------------------------------------------------------------
# Users can add apps they use frequently (e.g., "code", "spotify") to this list.
ALLOWED_APPS = [
    "calc", "notepad", "explorer",            # Windows defaults
    "calc.exe", "notepad.exe", "explorer.exe",
    "Calculator", "TextEdit", "Finder"        # macOS defaults
]
...
def run_action():
    ...
    try:
        ...
        if action == "open_app":
            ...
            # Check against the user-configurable allowlist
            if not any(app_name.lower() == allowed.lower() for allowed in ALLOWED_APPS):
                return jsonify({"success": False, "result": f"App '{app_name}' is not in the allowlist. To allow it, edit ALLOWED_APPS in app.py."}), 403
        ...
        # ── create_file ───────────────────────────────────────────────────
            ...
            base_dir = pathlib.Path(os.path.expanduser("~/PebbleAI_Files")).resolve()
            
            # Strip leading slashes to prevent raw_path from being treated as an absolute root
            # when appended to base_dir
            safe_raw = raw_path.lstrip("\\/")
            target_path = (base_dir / safe_raw).resolve()
            
            if not target_path.is_relative_to(base_dir):
                return jsonify({"success": False, "result": f"Access denied. Path must be within {base_dir}"}), 403
            ...
            with open(target_path, "w", encoding="utf-8") as f:
                f.write(content)
            return jsonify({"success": True, "result": f"File created: {target_path}"})
...
if __name__ == "__main__":
    detect_gpu()
    prewarm_models()
    app.run(debug=os.environ.get("PEBBLE_DEBUG") == "1", threaded=True, host="127.0.0.1")